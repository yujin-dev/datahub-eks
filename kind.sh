...
KIND_K8S_IMAGE=${KIND_K8S_IMAGE:-"kindest/node:v1.24.12"}
KIND_CLUSTER_NAME=${KIND_CLUSTER_NAME:-"datahub"}
...
cat <<EOF | kind create -v ${VERBOSE}  cluster --name="${KIND_CLUSTER_NAME}" --image="${KIND_K8S_IMAGE}" --wait="${KIND_WAIT}" 
EOF
